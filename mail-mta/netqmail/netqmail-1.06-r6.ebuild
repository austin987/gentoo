...
pkg_setup() {
	if [[ -n "${QMAIL_PATCH_DIR}" ]]; then
		...
		eerror "you should put them in /etc/portage/patches/"
		eerror "See https://wiki.gentoo.org/wiki//etc/portage/patches"
		eerror
...
src_prepare() {
	eapply "${FILESDIR}"/${PV}-exit.patch
	eapply "${FILESDIR}"/${PV}-readwrite.patch
	eapply "${DISTDIR}"/${QMAIL_LARGE_DNS}
	eapply "${FILESDIR}"/${PV}-fbsd-utmpx.patch
	...
	if ! use vanilla; then
		...
		sed 's~^--- ../../~--- ~g' \
			<"${DISTDIR}"/${QMAIL_TLS_F} \
			>"${T}"/${QMAIL_TLS_F} || die
		use ssl        && eapply "${T}"/${QMAIL_TLS_F}
		use ssl        && eapply "${DISTDIR}"/${QMAIL_TLS_CVE}
		use highvolume && eapply "${DISTDIR}"/${QMAIL_BIGTODO_F}

		if use qmail-spp; then
			if use ssl; then
				eapply "${QMAIL_SPP_S}"/qmail-spp-smtpauth-tls-20060105.diff
			else
				eapply "${QMAIL_SPP_S}"/netqmail-spp.diff
			fi
			cd "${WORKDIR}" || die
			eapply "${FILESDIR}"/genqmail-20080406-ldflags.patch
			cd - || die
		fi
	fi
	...
	eapply "${FILESDIR}"/qmail-smtputf8.patch
	cd - || die
